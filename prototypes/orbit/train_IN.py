...
if __name__ == "__main__":
    ...
    dataset = np.load("{}/dataset.npy".format(DATASET_FOLDER)).astype(np.float32)
    for fold in tqdm.trange(NUM_FOLDS, desc="Fold"):    
        ...
        validation = np.load("{}/{}.validation.npy".format(DATASET_FOLDER,fold))
            ...
            for b, batch in tqdm.tqdm(enumerate(get_epoch(dataset,train,BATCH_SIZE)), total=train.shape[0]/BATCH_SIZE, desc="Batch Train"):
                ...
                if False and epoch<NOISE_EPOCH_STOP_DECAY:
                    ...
                    if epoch>NOISE_EPOCH_START_DECAY:
                        proportion *= 1-((NOISE_EPOCH_STOP_DECAY-epoch)/(NOISE_EPOCH_STOP_DECAY-NOISE_EPOCH_START_DECAY))
                    #end if
                    idx = np.random.choice(noise.shape[0], replace=False, size=int(noise.shape[0] * (1-proportion)))
                    noise[idx] = 0
                ...
                if use_cuda:
                    bOin, bOout, bMsrc, bMtgt, n_list, m_list = map(lambda x: x.cuda(), (bOin, bOout, bMsrc, bMtgt, n_list, m_list))
