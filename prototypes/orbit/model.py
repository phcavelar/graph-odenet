...
class MLP(nn.Module):
    def __init__(self, input, layers, output):
        ...
        self.hidden_linear = [nn.Linear(d_i, d_o, bias=True)
                              for d_i, d_o in zip(ds[:-2], ds[1:-1])]
...
class IN(nn.Module):
    """
      ...
      Mtgt is the matrix indicating that object i is the target of relation r, of shape (n,m)
    ...
    def forward(self, O, R, X, Msrc, Mtgt):
        ...
        E_prime = torch.cat([torch.matmul(Mtgt, E)] +
                            ([X] if X is not None else []), dim=1)
        P = self.fO(E_prime)
        return P
# end IN
