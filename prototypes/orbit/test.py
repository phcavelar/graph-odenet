...
G = 6.67408e-11 * 100000000  # Otherwise the bodies would not move given the small value of gravitational constant
HIST_TIMESTEPS = 10
TIMESTEP_DELAY = 10
NUM_OF_BODIES = 5
TIME_DELTA = 0.01
...
def nbody(dt, pos, vel, mass, radii=None, out_pos=None, out_vel=None, force_placeholder=None, distance_placeholder=None, G=39.478):
    out_pos, out_vel, force_placeholder = map(lambda x: np.empty_like(pos) if x is None else x, (out_pos, out_vel, force_placeholder))
    ...
    for i in range(n):
        for j in range(n):
            if i!=j:
                force_placeholder[i] -= distance_placeholder[i,j,:]*(G*m[i]*m[j]/(np.linalg.norm(distance_placeholder[i,j,:])**2))
    #
...
p2 = np.random.uniform(low=min(WIDTH,HEIGHT)*1/7, high=min(WIDTH,HEIGHT)*1/4,size=(NUM_OF_BODIES,num_dims))
...
for i in range(NUM_OF_BODIES):
    ...
    v[i,0] *= np.sqrt(np.linalg.norm(centripetal_force[i])*np.linalg.norm(distance_to_center[i])/m[i])
    v[i,1] = centripetal_force[i,0]/np.linalg.norm(centripetal_force[i])
    v[i,1] *= np.sqrt(np.linalg.norm(centripetal_force[i])*np.linalg.norm(distance_to_center[i])/m[i])
