...
model_dict = {
        ...
        "eressum": UnimplementedModel,
        "eress2s": UnimplementedModel,
        "eodesum": UnimplementedModel,
        "eodes2s": UnimplementedModel,
}
...
def main():
    ...
    for tgt_idx, tgt in enumerate( dataset_targets[args.dataset] ):
        ...
        assert(len(l)==dataset_targets[args.dataset])
        g, h_t, e = g_tuple
        ...
        l_target = 1
        type ='regression'
        model = Model_Class(node_features=in_n, edge_features=in_e, target_features=l_target, hidden_features=hidden_state_size, num_layers=args.layers, dropout=0.5, type=type, s2s_processing_steps=args.s2s)
        del in_n, hidden_state_size, l_target, type
        ...
        for epoch in range(0, args.epochs):
            ...
            # train for one epoch
            train(train_loader, model, criterion, optimizer, epoch, evaluation, logger, target_range=(tgt_idx,))

            # evaluate on test set
            er1 = validate(valid_loader, model, criterion, evaluation, logger, target_range=(tgt_idx,))
        ...
        # For testing
        validate(test_loader, model, criterion, evaluation, target_range=(tgt_idx,))
    #end for tgt


def train(train_loader, model, criterion, optimizer, epoch, evaluation, logger, target_range=(0,-1)):
...
def validate(val_loader, model, criterion, evaluation, logger=None, target_range=(0,-1)):
    batch_time = AverageMeter()
    losses = AverageMeter()
    error_ratio = AverageMeter()
